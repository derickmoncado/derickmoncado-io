...
function copyImages() {
	...
	return src("src/assets/images/**/*.+(png|jpg|jpeg|gif|svg|pdf)")
...
function favicon() {
	console.log("----FAVICON!----");
	return src("src/favicon.ico").pipe(dest("dist")).pipe(browserSync.stream());
}
...
function minifyCss() {
	...
	return src(["dist/assets/css/main.css"])
		.pipe(sourcemaps.init())
