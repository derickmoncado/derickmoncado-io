...
(() => {
  ...
  const fetchSnippets = async () => {
    ...
    console.log(snippets); // Extract a couple values and then build out our HTML
  ...
  const fetchLatestVideo = async () => {
    ...
    try {
      ...
      const videoRes = await fetch(`https://www.googleapis.com/youtube/v3/playlistItems?part=snippet&playlistId=${uploadsPlaylistId}&maxResults=1&key=${API_KEY}`);
  ...
  const loadYouTubeSubscribeButton = () => {
    ...
    if (existingScript) existingScript.remove(); // Create and insert a new script tag
